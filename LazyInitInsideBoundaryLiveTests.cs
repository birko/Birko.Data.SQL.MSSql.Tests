...
/// <see cref="AbstractConnectorBase.SupportsTransactionalDdl"/>). SQL Server has transactional DDL and
...
///
/// <para>
/// Gated on <c>BIRKO_MSSQL_HOST</c> (+ <c>_PORT</c> / <c>_USER</c> / <c>_PASSWORD</c> / <c>_DB</c>), and a
...
public class LazyInitInsideBoundaryLiveTests : IDisposable
{
    ...
    private static AsyncMSSqlStore<LazyRow> AsyncStore()
    {
        var store = new AsyncMSSqlStore<LazyRow>();
        store.SetSettings(Settings());
        return store;
    }

    private static MSSqlStore<LazyRow> SyncStore()
    {
        var store = new MSSqlStore<LazyRow>();
        store.SetSettings(Settings());
        return store;
    }
    ...
    public async Task A_bulk_write_from_a_store_initialising_inside_the_boundary_still_rolls_back()
    {
        ...
        CommittedCount().Should().Be(0,
            "SQL Server has transactional DDL, so this always worked — it is pinned so that routing "
    ...
    /// </summary>
    /// <remarks>
    /// SQL Server's DDL is transactional, so the <c>CREATE TABLE</c> joins the caller's transaction and
    ...
    public async Task A_table_created_by_schema_ensure_is_rolled_back_with_the_boundary()
    {
        ...
        TableExists().Should().BeFalse(
            "SQL Server DDL is transactional, so the table went with the rollback — stated explicitly "
